...
if (!empty($_POST)){
	...
	if (empty($error_fields)){
                        ...
                        <td class="wrapper">
                          <table role="presentation" border="0" cellpadding="0" cellspacing="0">
                            <tr>
                              <td>
                                <p>Hi there,</p>
                                <p>I am '. $data["referred_by"] .' referring my friend who </p>
                                <p> have done '.  $data["qualification"] .' with an experience of '. $data["experience"].'
                                It would be a sincere pleasure to hear back from you soon to discuss the opportunity.</p>
                                <p>Look at the details below.</p>
                                <p> '. $data["fullname"] .'</p>
                                <p> '. $data["phone"] .'</p>
                                <p> '. $data["email"] .'</p>
                              </td>
		...
		// Additional headers
		$headers[] = 'From:'.$data['referred_by'].'<'.$data['referred_by_email'].'>';
		$insertdata = $dbConn->insert('contact',$data);
		// Send email
		 $check = 1 ; // mail($to, $subject, $message, implode("\r\n", $headers));
		 if($check)
		 	$res['mail']="sent";
		 else
			$res['mail']="not sent";
